#!/usr/bin/env python3
"""
AURA HP-App Funnel — Email Sync to ActiveCampaign (Automation 71)

Liest mail-N.md (Markdown + Frontmatter), rendert in die AURA Email-Vorlage
(Cream-Card + Gold-Bars + Jessie-Avatar), pusht via API zu AC.

Usage:
  python3 sync.py          # push to AC
  python3 sync.py --dry    # nur Preview, kein Push
  python3 sync.py --html   # schreibt preview-N.html zum Reinschauen
"""
...
LIVE_MSG_MAP = {1: 198, 2: 199, 3: 200, 4: 205, 5: 206, 6: 207}
LIVE_CMP_MAP = {1: 219, 2: 221, 3: 223, 4: 228, 5: 230, 6: 232}
...
# ===== TEMPLATE (gleich wie App-Welcome) =====
...
def ac(method, path, payload=None):
...
def main():
    ...
    for f in sorted(glob.glob(os.path.join(HERE, "mail-*.md"))):
        ...
        if not msg_id:
            print(f"  [skip] no message_id"); continue
        if dry:
            print(f"  [dry] would update message {msg_id}"); continue

        msg_payload = {"message": {
            "name": f"HP-App Mail {mail_n}",
            "subject": fm["subject"],
        ...
        if cid:
            try:
                ac("PUT", f"/api/3/campaigns/{cid}", {"campaign":{"name":f"HP-App Mail {mail_n}", "subject":fm["subject"]}})
            except: pass

    print("\nDone.")
    print("Builder: https://auraakademie.activehosted.com/app/builder/71")
